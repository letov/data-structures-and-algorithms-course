...
class EdgeListDirectGraph: EdgeList & DirectedGraph {
    ...
    override func hasEdge(vertex1: String, vertex2: String) -> Bool {
        for edgeVector in edgeVectors {
            if (edgeVector.vector.vertex1 == vertex1 && edgeVector.vector.vertex2 == vertex2) {
                return true
            }
        }
        return false
    }
    func getEdge(vertex1: String, vertex2: String) -> Edge? {
        for edgeVector in edgeVectors {
            if (edgeVector.vector.vertex1 == vertex1 && edgeVector.vector.vertex2 == vertex2) {
                return edgeVector.edge
            }
        }
        return nil
    }
