...
class GraphsTests: XCTestCase {
    ...
    func testAdjacencyVectorAdd() throws {
        XCTAssertEqual(avGraph.hasVertex(vertex: "A"), false)
        avGraph.addVertex(vertex: "A")
        XCTAssertEqual(avGraph.hasVertex(vertex: "A"), true)
        avGraph.addVertex(vertex: "A")
        avGraph.addVertex(vertex: "B")
        XCTAssertEqual(avGraph.hasEdge(vertex1: "A", vertex2: "B"), false)
        avGraph.addEdge(vertex1: "A", vertex2: "B", edge: EdgeMock())
        XCTAssertEqual(avGraph.hasEdge(vertex1: "A", vertex2: "B"), true)
    }
    
    func testAdjacencyVector() throws {
        var avGraph: Graph = avGraph
        fillGraph(graph: &avGraph)
        avGraph.printTable()
        XCTAssertEqual(avGraph.vertexDegree(vertex: "A"), 3)
        XCTAssertEqual(avGraph.vertexDegree(vertex: "C"), 2)
        XCTAssertEqual(avGraph.vertexDegree(vertex: "E"), 0)
        XCTAssertEqual(avGraph.getAdjacentEdge(vertex: "F").count, 2)
        XCTAssertEqual(avGraph.getAdjacentVertex(vertex: "D"), ["A","B"])
    }
